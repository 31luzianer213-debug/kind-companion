...
async function loadConfig(supabase: any, userId: string): Promise<LoadedConfig> {
  ...
  const url = String(db?.sigma_url ?? meta?.url ?? "").trim();
  const serverName = String(db?.sigma_server_name ?? meta?.server_name ?? "").trim();
  return {
    url,
    server_name: serverName,
    server_display_name: serverName || "Servidor Sigma",
    streaming_dns: String(db?.sigma_streaming_dns ?? meta?.streaming_dns ?? "").trim(),
    username: String(db?.sigma_username ?? meta?.username ?? "").trim(),
    password: String(db?.sigma_password ?? meta?.password ?? ""),
    token: db?.sigma_token ?? meta?.token ?? null,
    enabled: Boolean(db?.sigma_enabled ?? meta?.enabled ?? false),
    auto_renew: Boolean(db?.sigma_auto_renew ?? meta?.auto_renew ?? false),
    last_sync_at: db?.sigma_last_sync_at ?? meta?.last_sync_at ?? null,
  };
}

function hasSigmaAccess(config: SigmaConfig) {
  return Boolean(config.url?.trim() && (config.token?.trim() || (config.username?.trim() && config.password?.trim())));
}

function mergeInput(saved: LoadedConfig, input?: SigmaInput): SigmaConfig {
  return {
    url: input?.url?.trim() || saved.url,
    username: input?.username?.trim() || saved.username,
    password: input?.password ?? saved.password,
    token: input?.token?.trim() || saved.token,
  };
}
...
export const saveSigmaSettings = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const payload = {
      ...
      sigma_url: data.sigma_url.trim(),
      sigma_server_name: (data.sigma_server_name ?? "").trim(),
      sigma_streaming_dns: (data.sigma_streaming_dns ?? "").trim(),
      sigma_username: data.sigma_username.trim(),
      sigma_password: data.sigma_password ?? "",
      sigma_token: data.sigma_token?.trim() || null,
      sigma_enabled: Boolean(data.sigma_enabled),
      sigma_auto_renew: Boolean(data.sigma_auto_renew),
    };
    const result = await context.supabase.from("whatsapp_settings").upsert(payload, { onConflict: "user_id" });
...
export const testSigmaConnection = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((input: SigmaInput) => input ?? {})
  .handler(async ({ data, context }) => {
    const { fetchSigmaPanelDetails, listSigmaCustomers, sigmaLogin } = await import("./sigma.panel");
    const saved = await loadConfig(context.supabase, context.userId);
    ...
    try {
      ...
      const result = await context.supabase.from("whatsapp_settings").upsert(
        {
          ...
          sigma_token: token,
          sigma_enabled: true,
          ...(details.serverName ? { sigma_server_name: details.serverName } : {}),
          ...(details.dns ? { sigma_streaming_dns: details.dns } : {}),
        },
        { onConflict: "user_id" },
      );
      if (result.error) return { ok: false as const, error: result.error.message };
      ...
      return { ok: false as const, error: error instanceof Error ? error.message : "Não foi possível conectar ao painel." };
    }
  });
...
export async function runSigmaSyncForUser(supabase: any, userId: string, data?: SigmaInput) {
  ...
  if (!hasSigmaAccess(config)) {
    return { ok: false as const, created: 0, updated: 0, createdNames: [] as string[], error: "Painel Sigma não configurado. Teste a conexão ou preencha as credenciais." };
  }
  ...
  try {
    ...
    for (const customer of customers) {
      ...
      const dueDate = customer.dueDate?.slice(0, 10) || null;
      const dueDay = dueDate ? Number(dueDate.slice(8, 10)) : null;
      const dns = details.dns || saved.streaming_dns;
      const m3u = customer.m3uUrl || (dns && customer.username && customer.password ? generateM3uUrl(dns, customer.username, customer.password, "ts") : null);
      const base = {
        sigma_customer_id: customer.id,
        sigma_username: customer.username ?? null,
        sigma_synced_at: now,
        iptv_username: customer.username ?? null,
        ...
        ...(m3u ? { notes: `M3U: ${m3u}${customer.notes ? `\n${customer.notes}` : ""}` } : customer.notes ? { notes: customer.notes } : {}),
      };
      if (match) {
        const result = await supabase.from("clients").update({ ...base, status: isActiveStatus(customer.status) ? "active" : "inactive" }).eq("id", match.id).eq("user_id", userId);
        if (result.error) throw new Error(result.error.message);
        updated++;
      } else {
        const name = customer.name?.trim() || customer.username || "Cliente Sigma";
        const result = await supabase.from("clients").insert({ user_id: userId, name, phone: (customer.phone ?? "").replace(/\D/g, ""), status: isActiveStatus(customer.status) ? "active" : "inactive", monthly_fee: 35, ...base });
        if (result.error) throw new Error(result.error.message);
        created++;
...
export const syncSigmaClients = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((input: SigmaInput) => input ?? {})
  .handler(async ({ data, context }) => runSigmaSyncForUser(context.supabase, context.userId, data));
